...
def test_perform_ewm():
    ...
    featd, f0 = perform_ewm(featd, ['s'], [5, 10], folder='test_data', name='test')
...
def test_perform_ewm_load():
    ...
    try:
        ...
        pass
    featd1, f0 = perform_ewm(featd1, ['s'], [5, 10], folder='test_data', name='test2')
    featd2, f0 = perform_ewm(featd2, ['s'], [5, 10], folder='test_data', name='test2')

    featd3 = {
        'dtsi': dt.astype(np.int64),
        'dscode': dscode,
        's': serie,
        's_ewm5': np.concatenate([featd1['s_ewm5'], featd2['s_ewm5']])
    }
