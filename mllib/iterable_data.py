...
class ParquetIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for filename in file_list:
            ...
            # the dataLoader will handle the batch_size
            for row in df.itertuples(index=False, name=None):
                # row is a tuple like (x1, x2, ..., xN, label)
                features = np.array(row)[idx_feats]
                label = np.array(row)[idx_target]

                # Convert to Tensors
                # float32 is the default of pytorch
