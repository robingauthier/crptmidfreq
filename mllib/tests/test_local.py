...
# pytest ./crptmidfreq/mllib/tests/test_local.py --pdb --maxfail=1
...
def test_train():
    ...
    df['dscode'] = np.random.randint(0, 10, df.shape[0])
    n = df.shape[0]
    ...
    def model_gen():
        model = LGBMModel(num_features=numfeats)
        return model
    model = LocalModel2(model_generator=model_gen)
    model.fit(dftrain, target_col='forward_fh2')
