        ...
        if ts != g_last_ts:
            ...
            while j < i:
                ...
                if last_wgts[by2] > 0 and last_cnts[by2] >= mincnt:
                    vart = (last_sums2[by2]/last_wgts[by2] -
                            np.power(last_sums[by2]/last_wgts[by2], 2))
                    vart = vart * (last_cnts[by2] / (last_cnts[by2] - 1))
                    result[j] = np.sqrt(vart)
                else:
                    result[j] = 0.0
                j += 1
            # reset everything
...
class csStdStepper(BaseStepper):
    ...
    def update(self, dt, dscode, serie, by=None, wgt=None):
        ...
        return update_cs_std_values(
            ...
            self.last_timestamps, self.last_sums, self.last_sums2,
            self.last_wgts, self.last_cnts, self.mincnt
        )
