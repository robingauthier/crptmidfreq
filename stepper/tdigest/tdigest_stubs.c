...
// https://github.com/dask/crick/blob/main/crick/tdigest_stubs.c
 #include <stdlib.h>
 #include <float.h>
 #include <assert.h>
 ...
 CRICK_INLINE tdigest_t *tdigest_new(double compression) {
     ...
     T->total_weight = 0;
     T->ncentroids = 0;
     T->centroids = (centroid_t *)calloc(size, sizeof(centroid_t));
     if (T->centroids == NULL)
         goto fail;
     T->merge_centroids = (centroid_t *)calloc(size, sizeof(centroid_t));
     ...
     T->buffer_centroids = (centroid_t *)calloc(buffer_size, sizeof(centroid_t));
     if (T->buffer_centroids == NULL)
         goto fail;
     T->buffer_sort = (centroid_t *)calloc(buffer_size, sizeof(centroid_t));
