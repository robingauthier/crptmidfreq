...
#########################################################
cdef max_values(int64_t[:] codes,
               double[:] values,
               int64_t[:] timestamps,
               unordered_map[int64_t, MaxState]& state_map):
    """
    Expanding Max
    """
    ...
    for i in range(n):
        ...
        # Using operator[] will insert a default MaxState (with last_timestamp==0) if not present
...
cdef dict _umap_to_pydict_state(unordered_map[int64_t, MaxState]& state_map):
    cdef dict d = {}
    cdef unordered_map[int64_t, MaxState].iterator it = state_map.begin()
...
cdef unordered_map[int64_t, MaxState] _pydict_to_umap_state(dict d):
    cdef unordered_map[int64_t, MaxState] state_map
    cdef int64_t key
    cdef tuple t
    cdef MaxState s
...
#########################################################
# Optimized Cython MaxStepper
#########################################################
cdef class MaxStepper:
    cdef dict __dict__
    cdef unordered_map[int64_t, MaxState] state_map

    def __init__(self, folder="", name=""):
        """
        Initialize MaxStepper
        """
    ...
    def update(self, dt, dscode, serie):
        ...
        res = max_values(codes, values, ts_array, self.state_map)
        return np.array(res)