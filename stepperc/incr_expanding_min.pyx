...
import numpy as np
cimport numpy as np
from libc.math cimport isnan, fmin
...
#########################################################
cdef min_values(int64_t[:] codes,
               double[:] values,
               int64_t[:] timestamps,
               unordered_map[int64_t, MinState]& state_map):
    """
    Expanding Min
    """
    ...
    for i in range(n):
        ...
        # Using operator[] will insert a default MinState (with last_timestamp==0) if not present
...
cdef dict _umap_to_pydict_state(unordered_map[int64_t, MinState]& state_map):
    cdef dict d = {}
    cdef unordered_map[int64_t, MinState].iterator it = state_map.begin()
...
cdef unordered_map[int64_t, MinState] _pydict_to_umap_state(dict d):
    cdef unordered_map[int64_t, MinState] state_map
    cdef int64_t key
    cdef tuple t
    cdef MinState s
...
#########################################################
# Optimized Cython MinStepper
#########################################################
cdef class MinStepper:
    cdef dict __dict__
    cdef unordered_map[int64_t, MinState] state_map

    def __init__(self, folder="", name=""):
        """
        Initialize MinStepper
        """
        self.folder = os.path.join(get_feature_folder(), folder)
        self.name = name
        self.state_map = unordered_map[int64_t, MinState]()
    ...
    def update(self, dt, dscode, serie):
        ...
        res = min_values(codes, values, ts_array, self.state_map)
        return np.array(res)