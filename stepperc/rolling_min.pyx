...
#########################################################
# Update function for rolling min
#########################################################
cdef update_rolling_min(int64_t[:] timestamps,
                       int64_t[:] codes,
                       double[:] values,
                       unordered_map[int64_t, RollingState]& state_map,
                       int window):
    """
    Update rolling min values for each code.
    ...
    # Then compute the min for each position
    for i in range(n):
        code = codes[i]
        s = &state_map[code]
        
        # Calculate min of the rolling window
        current_min = 1e300  # A very high number to start
        has_value = False
        
        for j in range(window):
            if not isnan(s.values[j]):
                if not has_value or s.values[j] < current_min:
                    current_min = s.values[j]
                    has_value = True
        
        result[i] = current_min if has_value else np.nan
    
    return result

#########################################################
# Optimized Cython RollingMinStepper 
#########################################################
cdef class RollingMinStepper(RollingStepper):
    
    def update(self, dt, dscode, values):
        """
        Update rolling min values.
        ...
        res = update_rolling_min(ts_array, codes, vals, self.state_map, self.window)
        return np.array(res)